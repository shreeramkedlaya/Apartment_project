...
def create_booking(user, amenity, start_time, end_time, purpose):
    # prevent creating a booking if the timeslot overlaps with an APPROVED booking
    overlapping = AmenityBooking.objects.filter(
        amenity=amenity,
        status=AmenityBooking.Status.APPROVED,
...
def update_booking_status(booking, new_status, manager):
    ...
    if new_status == AmenityBooking.Status.APPROVED:
        overlapping = AmenityBooking.objects.filter(
    ...
    # send notification
    if new_status in [AmenityBooking.Status.APPROVED, AmenityBooking.Status.REJECTED]:
        send_notification_task.delay(
            title=f"Amenity Booking {new_status}",
